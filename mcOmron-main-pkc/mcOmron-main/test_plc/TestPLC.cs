...
namespace test_plc
{
	public partial class TestPLC : Form
	{
		...
		private void WriteDM()
		{
			...
			ushort startAddress = Convert.ToUInt16(dm_position.Text);

			try
			{
				Stopwatch stopwatch = new Stopwatch();
				string tmp = dm_value.Text;
				stopwatch.Start();

				foreach (var i in tmp)
				{
					dmval = Convert.ToUInt16(i);
					...
					if (!plc.WriteDM(startAddress, dmval))
					{
						throw new Exception(plc.LastError);
					}
				...
				stopwatch.Stop();
				Console.WriteLine("데이터 처리 시간: {0}ms", stopwatch.ElapsedMilliseconds);
		...
		private void WriteDMS()
		{
			...
			try
			{
				Stopwatch stopwatch = new Stopwatch();
				stopwatch.Start();
				// dm_position을 UInt16으로 변환
				ushort dmPosition = Convert.ToUInt16(dm_position.Text);

				
				string dmValuesText = dm_value.Text;
				ushort[] dmValues = new ushort[dmValuesText.Length];

				int i = 0;
				foreach (var tmp in dmValuesText)
				{
					dmValues[i] = Convert.ToUInt16(tmp);
					i++;
				}

				

				if (!plc.WriteDM(dmPosition, dmValues))
				{
					throw new Exception(plc.LastError);
				}
